...
1. **Labeled Data**:
    ...
    - Each training example has:
        ...
        - Input (features) – the data the model uses to make predictions.

        - Output (labels/targets) – the correct answer the model should predict.

    - Example: Predicting house prices:

        - Input: [number of rooms, area, location]

        - Output: [price of the house]

2. **Training**:

    - The model is fed the labeled data.

    - It tries to learn the relationship (function) between inputs and outputs.

    - It adjusts its internal parameters (like weights in neural networks) to minimize errors in prediction.

3. **Prediction (Inference)**:

    - Once trained, the model can take new inputs (without labels) and predict outputs.

4. **Error/ Loss Function**:

    - During training, the model evaluates its predictions against the actual labels.

    - It uses a loss function (like Mean Squared Error for regression or Cross-Entropy for classification) to measure how far off its predictions are and improve.
...
- Classification (predicting categories or classes):
    ...
    - Example: Email spam detection (spam or not), image recognition (cat, dog, or bird).
    - Common algorithms: Logistic Regression, Support Vector Machines, Neural Networks.
...
![Classification Diagram](../../images/supervisedLearning.png)
